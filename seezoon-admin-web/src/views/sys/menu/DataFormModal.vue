        ...
        <a-tab-pane v-for="pane in tabsPanes" :key="pane.key" :tab="pane.title">
          <div v-if="pane.key === dataForm.type">
            ...
            <a-row v-if="pane.key === 2">
              ...
              <a-col :md="12" :xs="24">
                <a-form-item :rules=" [{ required: true, message: '打开位置不能为空', whitespace: true }]"
                             label="打开位置" name="target">
                  <a-radio-group v-model:value="dataForm.target" defaultValue="main">
                    <a-radio-button value="main">
                      主窗口
                    </a-radio-button>
                    <a-radio-button value="_blank">
                      新窗口
                    </a-radio-button>
                  </a-radio-group>
                </a-form-item>
            ...
            <a-row>
              <a-col :md="12" :xs="24">
                <a-form-item
                    :rules="[]" label="权限" name="permission">
                  <a-input v-model:value="dataForm.permission" :maxlength="50" placeholder="权限标识"/>
                </a-form-item>
              </a-col>
              <a-col :md="12" :xs="24">
                <a-form-item :rules="[
              { required: true, type:'number', message: '请输入数字', whitespace: true },
            ]" help="顺序越小显示越前" label="顺序" name="sort">
                  <a-input-number v-model:value="dataForm.sort" :max="10000" :precision="0" :step="10"
                                  placeholder="请输入顺序"></a-input-number>
                </a-form-item>
              </a-col>
            </a-row>
            <a-row>
              <a-col :md="12" :xs="24">
                <a-form-item
                    :rules="[]" label="图标" name="icon">
                  <a-input
                      v-model:value="dataForm.icon" :maxlength="50" placeholder="选择图标">
                  </a-input>
                </a-form-item>
              </a-col>
              <a-col :md="12" :xs="24">
...
export default {
  ...
  data() {
    return {
      tabsPanes: [{key: 1, title: '目录'}, {key: 2, title: '菜单'}, {key: 3, title: '按钮'}]
    }
  },
  methods: {
    checkParamKey(rule, value) {
      return this.uniqueFieldSimpleValidation('/sys/menu/checkParamKey', value, {
        id: this.dataForm.id,
        paramKey: value
      }, `唯一键 ${value} 已存在`)
    },
