...
export default {
  ...
  methods: {
    checkUsername(rule, value) {
      return this.uniqueFieldSimpleValidation('/sys/user/checkUsername', value, {
        id: this.dataForm.id,
        username: value
      }, `登录名 ${value} 已存在`)
    },
    checkMobile(rule, value) {
      return this.uniqueFieldSimpleValidation('/sys/user/checkMobile', value, {
        id: this.dataForm.id,
        mobile: value
      }, `手机号 ${value} 已存在`)
    },
    checkPassword(rule, value) {
      return new Promise((resolve, reject) => {
        ...
        if (this.dataForm.password === this.dataForm.confirmPassword) {
          this.$refs.dataForm.clearValidate(rule.field === 'password' ? 'confirmPassword' : 'password');
          resolve()
        } else {
          reject('密钥和确认密码不一致')
        }
