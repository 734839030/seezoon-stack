...
SERVER_OTPS="--server.port=8080 --server.tomcat.connection-timeout=10 --server.tomcat.max-connections=2000 --server.tomcat.accept-count=1000 --server.tomcat.threads.min-spare=10 --server.tomcat.threads.max=2000 --server.shutdown=graceful --spring.lifecycle.timeout-per-shutdown-phase=10s"
