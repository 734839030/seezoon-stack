...
import com.seezoon.user.modules.security.handler.AjaxAuthenticationFailureHandler;
import com.seezoon.user.modules.security.handler.AjaxAuthenticationSuccessHandler;
import com.seezoon.user.modules.security.handler.AjaxLogoutSuccessHandler;
import com.seezoon.user.modules.security.handler.UserAccessDeniedHandler;
...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
    ...
    private final SeezoonUserProperties seezoonUserProperties;
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        if (seezoonUserProperties.getLogin().isConcurrentSessionControlEnabled()) {
            // seesion 管理 一个账号登录一次，maxSessionsPreventsLogin=false后面的挤掉前面的,true直接报错,需要添加下方httpSessionEventPublisher
            http.sessionManagement().maximumSessions(seezoonUserProperties.getLogin().getMaximumSessions())
                .maxSessionsPreventsLogin(seezoonUserProperties.getLogin().isMaxSessionsPreventsLogin())
        ...
        http.rememberMe().rememberMeParameter(DEFAULT_REMEMBER_ME_NAME)
            .key(seezoonUserProperties.getLogin().getRememberKey()).useSecureCookie(true)
            .tokenValiditySeconds((int)seezoonUserProperties.getLogin().getRememberTime().toSeconds())
     ...
     */
    @Bean
    @ConditionalOnProperty(name = "seezoon.user.login.concurrent-session-control-enabled", havingValue = "true")
    ...
    public void configure(AuthenticationManagerBuilder auth) throws Exception {
        ...
        auth.userDetailsService(adminUserDetailsService()).passwordEncoder(new UserPasswordEncoder());
    }
    ...
    @Bean
    public UserDetailsServiceImpl adminUserDetailsService() {
